...
from panda_gym.objects import YCBObject, RBOObject

YCB_Objects = os.listdir("/projects/collab/Sagar/robustness/state_test_panda_gym/panda-gym/panda_gym/assets/ycb")
RBO_Objects = os.listdir("/projects/collab/Sagar/robustness/state_test_panda_gym/panda-gym/panda_gym/assets/rbo")
...
class PyBullet:
    ...
    def _create_geometry(
        ...
        self.physics_client.addUserData(self._bodies_idx[body_name], "body_name", body_name)
    ...
    def create_plane(self, z_offset: float) -> None:
        ...
        # )
        self.physics_client.loadURDF(os.path.join(self.urdfRootPath, 'plane.urdf'), basePosition=[0, 0, -0.65])
    ...
    def create_table(
        ...
        if domain_randomize:
            ...
            self.physics_client.loadURDF(os.path.join(self.urdfRootPath, 'table/table.urdf'), basePosition=[x_offset, 0.0, -0.65], globalScaling=1)

    def create_object(
            self,
            body_name: str, 
            scale: Optional[int] = 1,
            position: Optional[np.ndarray] = np.array([0., 0., 0.]), 
            orientation: Optional[np.ndarray] = np.array([0., 0., 1., 0.])
    ) -> None:
        """Create object from urdf file.

        Args:
            body_name (str): The name of the body. Must be unique in the sim.
            position (np.ndarray, optional): The position, as (x, y, z). Defaults to [0, 0, 0].
            orientation (np.ndarray, optional): The orientation, as (x, y, z, w). Defaults to [0, 0, 1, 0].
        """
        if body_name in YCB_Objects:
            obj = YCBObject(body_name, client=self.physics_client)
        if body_name in RBO_Objects:
            obj = RBOObject(body_name, scale, client=self.physics_client)
        obj.load()
        self.physics_client.resetBasePositionAndOrientation(obj.body_id, position, orientation)
        self.objects_list[body_name] = obj
        return obj
    ...
    def get_object_state(
        ...
        """Read and return state of object created with urdf file.

        Args:
            body_name (str): The name of the body. Must be unique in the sim.
        """
        state = self.physics_client.getJointState(self.objects_list[body_name].body_id, joint_num)[0]
    ...
    def set_object_state(
        ...
        self.physics_client.resetBasePositionAndOrientation(self.objects_list[body_name].body_id, position, orientation)
        if body_name == 'ikeasmall':
            self.physics_client.resetJointState(self.objects_list[body_name].body_id, 2, 0.05)
            self.physics_client.resetJointState(self.objects_list[body_name].body_id, 0, 0.0)
        if body_name == 'microwave':
            self.physics_client.resetJointState(self.objects_list[body_name].body_id, 1, 0.15)
            self.physics_client.resetJointState(self.objects_list[body_name].body_id, 0, 0.0)
        if body_name == 'door':
            self.physics_client.resetJointState(self.objects_list[body_name].body_id, 1, 0.15)
            self.physics_client.resetJointState(self.objects_list[body_name].body_id, 0, 0.0)
